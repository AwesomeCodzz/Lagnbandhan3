...
class LanguageManager {
    ...
    async init() {
        try {
            ...
            if (!langToggle) {
                throw new Error('Language toggle button (#langToggle) not found in DOM');
            }
            if (!langText) {
    ...
    updateLanguageToggleText() {
        ...
        if (langText) {
            ...
            if (translations && translations.langText) {
                ...
                console.log(`✅ Updated language toggle: "${oldText}" → "${translations.langText}"`);
            } else {
                console.warn('❌ No langText translation found for:', this.currentLanguage);
            }
        } else {
    ...
    updateTexts() {
        ...
        Object.keys(translations).forEach(key => {
            ...
            if (element) {
                ...
                console.log(`✅ Updated #${key}: "${oldText}" → "${translations[key]}"`);
                updatedCount++;
            } else {
        ...
        if (missingElements.length > 0) {
            console.warn('❌ Elements not found for translation keys:', missingElements);
        ...
        console.log(`✅ Updated ${updatedCount} text elements for language: ${this.currentLanguage}`);
    }
    ...
    updateSelectOptions() {
        ...
        const profileOptions = [
            'selectProfileOption', 'selfOption', 'sonOption', 'daughterOption',
            'brotherOption', 'sisterOption', 'relativeOption', 'friendOption'
        ];
    ...
    updatePlaceholders() {
        ...
        const inputsWithPlaceholders = document.querySelectorAll('[data-placeholder-key]');
